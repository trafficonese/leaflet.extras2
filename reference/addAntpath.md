...
to adapt the Antpath behaviour. See
...
Unlike
[`addPolylines`](https://rstudio.github.io/leaflet/reference/map-layers.html),
antpath layers are not registered as `"shape"`. In Shiny the mouse
events therefore use the `antpath` category instead of `shape`:

1.  A click updates `input$MAPID_antpath_click`

2.  A mouseover updates `input$MAPID_antpath_mouseover`

3.  A mouseout updates `input$MAPID_antpath_mouseout`
