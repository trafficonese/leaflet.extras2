...
It is possible to style the lines and the arrowheads separately; put
styling specifications for the lines inside `addArrowhead` and the
specifications for the arrowheads inside
[`arrowheadOptions`](https://trafficonese.github.io/leaflet.extras2/reference/arrowheadOptions.md).
A dashArray provided for the lines will also apply to the arrowheads,
but the latter can be overruled by specifying a separate dashArray for
